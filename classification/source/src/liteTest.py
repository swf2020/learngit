...
def load_data(keyStrs): #返回的是数据字典
    read = pd.read_csv("data_1.csv", low_memory=False)
    row_count = len(read)  # 行数
    col_count = len(read.columns)  # 列数
    print("列数：",col_count)
    print("行数：",row_count)
    time_dic = {} #建立一个存储数据的总字典
    for index in range(0,row_count): #建立字典的键
        if read['time'][index] not in  time_dic.keys(): #是否有对应的键
            time_dic.setdefault(read['time'][index], copy.deepcopy(keyStrs)) #没有就添加新的键值对
        if read['name3'][index] in keyStrs.keys(): #值是否存在
            time_dic[read['time'][index]][read['name3'][index]] = read['value'][index] #不存在就加入相应的值
    liContent = []
...
if __name__ == '__main__':
    ...
    data_dic, liContent = load_data(keyStrs)  # ,参数是一个字典
    # 写excel文件
